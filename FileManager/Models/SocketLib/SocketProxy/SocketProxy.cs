...
using FileManager.Models.SocketLib.SocketServer;
...
namespace FileManager.Models.SocketLib
{
    ...
    public class SocketProxy : SocketServerBase
    {
        ...
        private byte[] ReceiveProxyHeader(Socket client)
        {
            ...
            SocketIO.SocketIO.ReceiveBuffer(client, bytes, 2);
            return bytes;
        }
        ...
        */
        ...
        private SocketSender AuthenticationProxy(Socket client)
        {
            // todo 重写 21.05.28
            //byte[] proxy_header;
            SocketIO.SocketIO.ReceiveBytes(client, out HB32Header route_header, out byte[] route_bytes);
